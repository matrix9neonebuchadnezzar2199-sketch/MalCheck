...
# @name auto_analyze
# @description Export metadata, decompilation, and heuristics to JSON (MalCheck: /output/analysis.json)
...
def _job_id_for_output():
    """postScript CLI args [job_id, output.json], then CYBERGHIDRA_JOB_ID env."""
    try:
        args = getScriptArgs()
        if args is not None and len(args) > 0:
            a0 = str(args[0])
            if not a0.endswith(".json"):
                return a0
    except NameError:
        pass
    except Exception:
...
def _output_path_for_write():
    """MalCheck: args [job_id, path] or [path]; default /output/analysis.json."""
    try:
        args = getScriptArgs()
...
def run():
    ...
    out_path = _output_path_for_write()
    try:
        _progress_emit(98)
